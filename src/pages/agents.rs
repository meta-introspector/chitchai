...
pub fn AgentGrid() -> Element {
    rsx! {
        div { class: "bg-slate-800 min-h-screen p-8",
            div { class: "container mx-auto",
                div { class: "grid grid-cols-1 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-8",
                    ...
                    AddAgentCard { on_click: move |_| log::info!("clicked adding agent!") }
                }
            }
        }
    }
...
#[component]
pub fn AddAgentCard(on_click: EventHandler<MouseEvent>) -> Element {
    rsx! {
        div {
...
pub fn AgentCard(props: AgentCardProps) -> Element {
    //
    rsx! {
        div { class: "flex w-full max-w-md flex-col rounded-3xl bg-slate-50 p-8 text-slate-900 ring-1 ring-slate-300 \
            dark:bg-slate-900 dark:text-slate-200 dark:ring-slate-300/20",
            div { class: "flex justify-between items-center",
                h3 { class: "text-lg font-semibold leading-8", "{props.name}" }
                img {
                    class: "h-16 w-16 rounded-full object-cover",
                    src: props.img_url.as_str(),
                    alt: props.name.as_str(),
                }
            }
            div { class: "mt-4",
            ...
            button { class: "mt-4 rounded-md bg-blue-600 text-white px-4 py-2 text-sm font-semibold leading-5 \
