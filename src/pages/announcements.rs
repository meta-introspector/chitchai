...
pub fn AnnouncementPage() -> Element {
    ...
    let messages = announcement
        ...
        .map(|a| {
            let command = format!(
                "Please help me write an announcement with title `{}`",
                a.title
            );
    ...
    rsx! {
        div { class: "flex h-screen w-screen flex-col relative",
            div { class: "flex flex-col h-full space-y-6 bg-slate-200 text-sm leading-6 text-slate-900 shadow-sm dark:bg-slate-900 dark:text-slate-300 sm:text-base sm:leading-7",
                div { class: "overflow-auto max-h-[100vh] flex-grow dark:scrollbar dark:scrollbar-thumb-slate-700 dark:scrollbar-track-slate-900",
                    {messages.into_iter().map(|msg| rsx! {
                        MessageCard { chat_msg: msg }
                    })}
                }
            }
        }
