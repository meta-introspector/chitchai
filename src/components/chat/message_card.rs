...
pub fn MessageCard(chat_msg: ChatMsg) -> Element {
    let chat_msg = &chat_msg;
    match &chat_msg.msg {
        ChatCompletionRequestMessage::System(sys_msg) => {
            // let content = sys_msg.content.as_str();
            let content = "test system message"; // FIXME: fix this after testing
            rsx! {
                div { class: "flex flex-row-reverse items-start p-5",
                    img {
        ...
        ChatCompletionRequestMessage::Assistant(assistant_msg) => {
            ...
            //     .as_str();
            let content = "test assistant message"; // FIXME: fix this after testing
            let name_char = assistant_msg
                .name
                .as_ref()
                .map(|name| name.as_str().chars().next().unwrap())
                .unwrap_or('A');
...
#[component]
pub fn MarkdownTextBox(content: String) -> Element {
    rsx! {
        div { class: "flex min-h-[85px] rounded-b-xl rounded-tl-xl bg-slate-50 px-4 dark:bg-slate-800 sm:min-h-0 sm:max-w-md md:max-w-2xl",
