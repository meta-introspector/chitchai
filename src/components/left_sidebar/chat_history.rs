...
pub fn ChatHistorySidebar() -> Element {
    ...
    let chats: Vec<(String, DatetimeString, Uuid)> = use_context::<StoredStates>()
        .chats
        .iter()
            ...
            div { class: "mx-2 mt-8 space-y-4",
                // chat list
                {
                    chats
                        .into_iter()
                        .rev()
                        .map(|(title, date, id)| rsx! {
                            ChatHistoryItem {
                                onclick: move |_| { chat_event_handler.send(LeftSidebarEvent::ChangeChat(id)) },
                                title,
                                date: date.0,
                            }
                        })
...
#[component]
pub fn ChatHistoryItem(title: String, date: String, onclick: EventHandler<MouseEvent>) -> Element {
    rsx! {
        button {
            onclick: move |event| {
                onclick.call(event);
            ...
            p { class: "text-xs text-slate-500 dark:text-slate-400", "{date}" }
        }
    }
}
