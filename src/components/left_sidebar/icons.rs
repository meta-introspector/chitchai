...
pub fn IconSidebar() -> Element {
    let now_active_secondary = use_context::<SecondarySidebar>();
    ...
    rsx! {
        div { class: "flex h-screen w-12 flex-col items-center space-y-8 border-r border-slate-300 bg-slate-50 py-8 dark:border-slate-700 dark:bg-slate-900 sm:w-16",
            Logo {}
            NewConversationButton { activatable: false, active: false }
            ConversationListButton { activatable: true, active: conversation_list_button_active }
            DiscoverButton { activatable: false, active: false }
            UserProfileButton { activatable: true, active: user_profile_button_active }
            SettingsButton { activatable: false, active: false }
        }
    }
}
...
pub fn ConversationListButton(props: ButtonProps) -> Element {
    ...
    rsx! {
        RawButton {
            on_click_active: move |_| {
                chat_sidebar_event_handler
                    .send(LeftSidebarEvent::DisableSecondary(SecondarySidebar::History))
            },
            on_click_inactive: move |_| {
                chat_sidebar_event_handler
                    .send(LeftSidebarEvent::EnableSecondary(SecondarySidebar::History))
            },
            button_props: props,
            svg {
                xmlns: "http://www.w3.org/2000/svg",
