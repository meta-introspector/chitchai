...
async fn setting_event_handler(
    mut rx: UnboundedReceiver<SettingEvent>,
            ...
            SettingEvent::SaveServiceConfig(openai_model) => {
                ...
                match gpt_service {
                    ...
                    Some(gpt_service) => {
                        ...
                        // save configs
                        let (new_auth, new_authed_client): (Auth, Client<Arc<dyn Config>>) =
                            match gpt_service {
                                GPTService::AzureOpenAI => {
                                    let auth = Auth::AzureOpenAI {
                                        api_version: service_settings
                                            .api_version
                                            .to_owned()
                                            .unwrap(),
                                        deployment_id: service_settings
                                            .deployment_id
                                            .to_owned()
                                            .unwrap(),
...
pub fn SettingSidebar() -> Element {
    // get global states
    let global = use_context::<StoredStates>(); // TODO: avoid implicit clone
    let authed_client = use_context::<AuthedClient>();
    // setup local states
    let enable_group_chat = use_signal(|| false);
    // setup shared states
    use_context_provider(ServiceSettings::default);
    let service_settings = use_context::<ServiceSettings>();
    use_coroutine(|rx| {
    ...
    rsx! {
        aside { class: "flex",
            div { class: "relative h-screen w-full overflow-y-auto border-l border-slate-300 bg-slate-50 py-8 dark:border-slate-700 dark:bg-slate-900 sm:w-full",
                div { class: "mb-4 flex items-center gap-x-2 px-2 text-slate-800 dark:text-slate-200",
                    CloseSettingButton {}
                    h2 { class: "text-lg font-medium", "Settings" }
                }
                ToggleGroupChat {}
                ServiceConfigs {
                    gpt_service: global.selected_service.clone(),
...
fn CloseSettingButton() -> Element {
    ...
    rsx! {
        button {
            ...
            span { class: "sr-only", "Close settings sidebar" }
        }
    }
}
            ...
            select {
                ...
                onchange: move |select| {
                    ...
                    match value.as_str() {
                        "AzureOpenAI" => {
                            setting_event_handler
                                .send(SettingEvent::SelectService(Some(GPTService::AzureOpenAI)))
                        }
                        "OpenAI" => {
                            setting_event_handler
                                .send(SettingEvent::SelectService(Some(GPTService::OpenAI)))
                        }
                ...
                option { value: "", "{NULL_OPTION}" }
                option { value: "AzureOpenAI", "Azure OpenAI" }
...
fn ServiceConfigs(props: ServiceConfigsProps) -> Element {
    ...
    let service_event_handler = use_coroutine(|mut rx| {
        ...
        async move {
            while let Some(event) = rx.next().await {
                match event {
                    ServiceEvent::SaveConfigs => setting_event_handler.send(
                        SettingEvent::SaveServiceConfig(service_settings.openai_model.clone()),
                    ),
                    ServiceEvent::SelectOpenAIModel(model) => {
                        service_settings.write().openai_model = model
                    }
                }
            }
        }
    ...
    rsx! {
        div { class: "my-4 border-t border-slate-300 px-2 py-4 text-slate-800 dark:border-slate-700 dark:text-slate-200",
            ...
            SelectServiceSection {}
            {
                if let Some(gpt_service) = props.gpt_service {
                    rsx! {
                        SecretInputs { gpt_service }
                        {
                            if gpt_service == GPTService::OpenAI {
                                rsx! {
                                    SelectOpenAIModel { enable_group_chat: props.enable_group_chat }
                                }
                            } else {
                                rsx! {
                                    div {}
                        ...
                        button {
                            ...
                            onclick: |_| { service_event_handler.send(ServiceEvent::SaveConfigs) },
                            "Save Configs"
                        }
                    }
                } else {
...
fn SecretInputs(gpt_service: GPTService) -> Element {
    ...
    match gpt_service {
        GPTService::OpenAI => rsx! {
            div {
                // API Key
                label { r#for: "{API_KEY}", class: "{LABEL_STYLE}", "API Key" }
                input {
                    r#type: "password",
                    id: "{API_KEY}",
                    class: "{INPUT_STYLE}",
                ...
                // Base URL
                label { r#for: "{API_BASE}", class: "{LABEL_STYLE}", "Base URL / API Base (Optional)" }
                input {
                    r#type: "url",
                    id: "{API_BASE}",
                    class: "{INPUT_STYLE}",
                ...
                // Org ID
                label { r#for: "{ORG_ID}", class: "{LABEL_STYLE}", "Org ID (Optional)" }
                input {
                    r#type: "text",
                    id: "{ORG_ID}",
                    class: "{INPUT_STYLE}",
        ...
        GPTService::AzureOpenAI => rsx! {
            div {
                ...
                // Base URL
                label { r#for: "{API_BASE}", class: "{LABEL_STYLE}", "Base URL / API Base" }
                input {
                    r#type: "url",
                    id: "{API_BASE}",
                    class: "{INPUT_STYLE}",
                ...
                // Deployment ID
                label { r#for: "{DEPLOYMENT_ID}", class: "{LABEL_STYLE}", "Deployment ID" }
                input {
                    r#type: "text",
                    id: "{DEPLOYMENT_ID}",
                    class: "{INPUT_STYLE}",
                ...
                // API Version
                label { r#for: "{API_VERSION}", class: "{LABEL_STYLE}", "API Version" }
                input {
                    r#type: "text",
                    id: "{API_VERSION}",
                    class: "{INPUT_STYLE}",
                ...
                onchange: move |change| {
                    ...
                    if model == NULL_OPTION {
                        ...
                        match usable_models.iter().find(|m| (*m).eq(model.as_str())).cloned() {
                            Some(m) => {
                                service_event_handler.send(ServiceEvent::SelectOpenAIModel(Some(m)))
                            }
...
fn ModelParameters() -> Element {
    ...
    rsx! {
        div { class: "my-4 border-t border-slate-300 px-2 py-4 text-slate-800 dark:border-slate-700 dark:text-slate-200",
            ...
            label { r#for: "max-tokens", class: "{LABEL_STYLE}", "Max tokens" }
            input {
                r#type: "number",
                id: "max-tokens",
                class: "{INPUT_STYLE}",
            ...
            label { r#for: "model-temperature", class: "{LABEL_STYLE}", "Temperature" }
            input {
                r#type: "number",
